...
class Handler extends \Icms\Core\EntityHandler {
	...
	public function &create($isNew = true) {
		$perm = new \Icms\Member\Groupperm\Entity();
		if ($isNew) {
			$perm->setNew();
		}
		return $perm;
	...
	public function deleteByGroup($gperm_groupid, $gperm_modid = null) {
		$criteria = new \Icms\Db\Criteria\Compo(new \Icms\Db\Criteria\Item('gperm_groupid', (int) ($gperm_groupid)));
		if (isset($gperm_modid)) {
			$criteria->add(new \Icms\Db\Criteria\Item('gperm_modid', (int) $gperm_modid));
		}
	...
	public function deleteByModule($gperm_modid, $gperm_name = null, $gperm_itemid = null) {
		$criteria = new \Icms\Db\Criteria\Compo(new \Icms\Db\Criteria\Item('gperm_modid', (int) $gperm_modid));
		if (isset($gperm_name)) {
			$criteria->add(new \Icms\Db\Criteria\Item('gperm_name', $gperm_name));
			if (isset($gperm_itemid)) {
				$criteria->add(new \Icms\Db\Criteria\Item('gperm_itemid', (int) $gperm_itemid));
			}
		}
		return $this->deleteAll($criteria);
	}
	...
	public function checkRight($gperm_name, $gperm_itemid, $gperm_groupid, $gperm_modid = 1, $webmasterAlwaysTrue=true) {
		$criteria = new \Icms\Db\Criteria\Compo(new \Icms\Db\Criteria\Item('gperm_modid', $gperm_modid));
		$criteria->add(new \Icms\Db\Criteria\Item('gperm_name', $gperm_name));
		$gperm_itemid = (int) $gperm_itemid;
		if ($gperm_itemid > 0) {
			$criteria->add(new \Icms\Db\Criteria\Item('gperm_itemid', $gperm_itemid));
		}
		if (is_array($gperm_groupid)) {
			...
			foreach ( $gperm_groupid as $gid) {
				$criteria2->add(new \Icms\Db\Criteria\Item('gperm_groupid', $gid), 'OR');
			}
			...
			$criteria->add(new \Icms\Db\Criteria\Item('gperm_groupid', $gperm_groupid));
		}
	...
	public function getItemIds($gperm_name, $gperm_groupid, $gperm_modid = 1) {
		$ret = array();
		$criteria = new \Icms\Db\Criteria\Compo(new \Icms\Db\Criteria\Item('gperm_name', $gperm_name));
		$criteria->add(new \Icms\Db\Criteria\Item('gperm_modid', (int) $gperm_modid));
		if (is_array($gperm_groupid)) {
			$criteria2 = new \Icms\Db\Criteria\Compo();
			foreach ( $gperm_groupid as $gid) {
			...
			$criteria->add(new \Icms\Db\Criteria\Item('gperm_groupid', (int) $gperm_groupid));
		}
