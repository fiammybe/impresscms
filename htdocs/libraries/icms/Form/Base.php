...
abstract class Form
{
	...
	public function addElement(
		...
		if ($formElement instanceof \Stringable) {
			...
			if (!$formElement->isContainer()) {
				...
				$requiredElements = $formElement->getRequired();
				$count = count($requiredElements);
				for ($i = 0; $i < $count; $i++) {
					$this->_required[] = $requiredElements[$i];
				}
			}
		}
	}
	 ...
	 * @return array   array of {@link Element} objects
	 */
	...
	/**
	 * get a reference to a {@link Element} object by its "name"
	 *
	 * @param string  $name	"name" attribute assigned to a {@link Element}
	 * @return Element|null  reference to a {@link Element}, null if not found
	 */
	 ...
	 */
	abstract public function insertBreak(?string $extra = null);
	...
	/**
	 * Renders the Javascript function needed for client-side form validation
	 *
