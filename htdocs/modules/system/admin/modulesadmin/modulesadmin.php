...
function xoops_module_install($dirname) {
	...
	if ($module_handler->getCount(new icms_db_criteria_Item('dirname', $dirname)) == 0) {
		...
		$msgs = array();
		$msgs[] = '<h4 style="text-align:' . _GLOBAL_LEFT . ';margin-bottom: 0px;border-bottom: dashed 1px #000000;">' . _MD_AM_INSTALLING . $module->getInfo('name') . '</h4>';
		...
		$msgs[] = '';
		$errs[] = '<h4 style="text-align:' . _GLOBAL_LEFT . ';margin-bottom: 0px;border-bottom: dashed 1px #000000;">' . _MD_AM_INSTALLING . $module->getInfo('name') . '</h4>';
			...
			if (!$module_handler->insert($module)) {
				...
				if ($blocks !== FALSE) {
					...
					foreach ($blocks as $blockkey => $block) {
						...
						if (!$db->query($sql)) {
							$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_BLOCKS_ADD_FAIL . '</span>', '<strong>' . $block['name'] . '</strong>', '<strong>' . $db->error() . '</strong>');
						} else {
							if (empty($newbid)) {
							...
							if ($template != '') {
								...
								if (!$tplfile_handler->insert($tplfile)) {
									...
									// generate compiled file
									if (!$icmsAdminTpl->template_touch($newtplid)) {
										$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_TEMPLATE_COMPILE_FAIL . '</span>', '<strong>' . $block['template'] . '</strong>', '<strong>' . icms_conv_nr2local($newtplid) . '</strong>');
									} else {
				...
				if ($module->getVar('hasnotification') != 0) {
					...
					$options = array('_NOT_CONFIG_DISABLE' => XOOPS_NOTIFICATION_DISABLE, '_NOT_CONFIG_ENABLEBLOCK' => XOOPS_NOTIFICATION_ENABLEBLOCK, '_NOT_CONFIG_ENABLEINLINE' => XOOPS_NOTIFICATION_ENABLEINLINE, '_NOT_CONFIG_ENABLEBOTH' => XOOPS_NOTIFICATION_ENABLEBOTH);
		...
		if ($pages > 0) {
			...
			foreach ($pages as $page) {
				if (!$page_handler->delete($page)) {
					$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_SYMLINK_DELETE_FAIL . '</span>', $page->getVar('page_title'), '<strong>' . $page->getVar('page_id') . '</strong>');
				} else {
			...
			if ($tcount > 0) {
				...
				for ($i = 0; $i < $tcount; $i++ ) {
					if (!$tplfile_handler->delete($templates[$i])) {
						$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_TEMPLATE_DELETE_FAIL . '</span>', $templates[$i]->getVar('tpl_file'), '<strong>' . icms_conv_nr2local($templates[$i]->getVar('tpl_id')) . '</strong>');
					} else {
			...
			if (is_array($block_arr)) {
				...
				for ($i = 0; $i < $bcount; $i++ ) {
					if (!$icms_block_handler->delete($block_arr[$i])) {
						$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_BLOCK_DELETE_FAIL . '</span>', '<strong>' . $block_arr[$i]->getVar('name') . '</strong>', '<strong>' . icms_conv_nr2local($block_arr[$i]->getVar('bid')) . '</strong>');
					} else {
			...
			if ($modtables !== FALSE && is_array($modtables)) {
				...
				foreach ($modtables as $table) {
					...
					if (!in_array($table, $reservedTables)) {
						...
						if (!$db->query($sql)) {
							$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_MOD_TABLE_DELETE_FAIL . '</span>', '<strong>' . $db->prefix($table) . '<strong> . ');
						} else {
				...
				if ($confcount > 0) {
					...
					for ($i = 0; $i < $confcount; $i++ ) {
						if (!$config_handler->deleteConfig($configs[$i])) {
							$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_CONFIGOPTION_DELETE_FAIL . '</span>', '<strong>' . icms_conv_nr2local($configs[$i]->getvar('conf_id')) . '</strong>');
						} else {
...
function icms_module_update($dirname) {
	...
	if (!$module_handler->insert($module)) {
					...
					if ($fcount == 0) {
						...
						if (!$result) {
							...
							foreach ($groups as $mygroup) {
								...
								$bperm->setVar('gperm_modid', 1);
								if (!$gperm_handler->insert($bperm)) {
									$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_BLOCK_ACCESS_FAIL . '</span>', '<strong>' . $newbid . '</strong>', '<strong>' . $mygroup . '</strong>');
								} else {
							...
							if ($template != '') {
								...
								if (!$tplfile_handler->insert($tplfile)) {
									$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_TEMPLATE_INSERT_FAIL . '</span>', '<strong>' . $blocks[$i]['template'] . '</strong>');
								} else {
									$newid = $tplfile->getVar('tpl_id');
							...
							$msgs[] = sprintf('&nbsp;&nbsp;' . _MD_AM_BLOCK_CREATED, '<strong>' . $blocks[$i]['name'] . '</strong>', '<strong>' . $newbid . '</strong>');
			...
			foreach ($block_arr as $block) {
				if (!in_array($block->getVar('show_func'), $showfuncs) || !in_array($block->getVar('func_file'), $funcfiles)) {
					...
					if (!icms::$xoopsDB->query($sql)) {
						$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_BLOCK_DELETE_FAIL . '</span>', '<strong>' . $block->getVar('name') . '</strong>', '<strong>' . $block->getVar('bid') . '</strong>');
					} else {
		...
		if ($confcount > 0) {
			...
			for ($i = 0; $i < $confcount; $i++ ) {
				if (!$config_handler->deleteConfig($configs[$i])) {
					$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_CONFIGOPTION_DELETE_FAIL . '</span>', '<strong>' . $configs[$i]->getvar('conf_id') . '</strong>');
			...
			foreach ($configs as $config) {
				...
				if (!in_array($config['name'], $config_delng)) {
					...
					if (FALSE !== $config_handler->insertConfig($confobj)) {
						$msgs[] = sprintf('&nbsp;&nbsp;' . _MD_AM_CONFIG_ADDED, '<strong>' . $config['name'] . '</strong>. ') . $confop_msgs;
					} else {
						$msgs[] = sprintf('&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_CONFIG_ADD_FAIL . '</span>', '<strong>' . $config['name'] . '</strong>. ');
					}
					unset($confobj);
